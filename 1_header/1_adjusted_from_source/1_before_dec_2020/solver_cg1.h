...
#ifndef dealii__solver_cg1_h
#define dealii__solver_cg1_h
...
class SolverCG1 : public Solver<VectorType>
{
public:
  /**
  ...
  double stopping_criterion = 1e-2;
...
inline
SolverCG1<VectorType>::AdditionalData::
AdditionalData (const bool log_coefficients,
...
template <typename VectorType>
SolverCG1<VectorType>::SolverCG1 (SolverControl        &cn,
                                VectorMemory<VectorType> &mem,
...
{
//     std::cout << "    I'm in the first constructor of SolverCG1" << std::endl;
}
...
SolverCG1<VectorType>::SolverCG1 (SolverControl        &cn,
  ...
  additional_data(data)
{
    std::cout << "        constructor of SolverCG1 receives SolverControl";
    stopping_criterion = cn.tolerance();
    std::cout << ", the stopping criterion is " << stopping_criterion << "\n ";
}



template <typename VectorType>
SolverCG1<VectorType>::~SolverCG1 ()
{}
...
template <typename VectorType>
void
SolverCG1<VectorType>::print_vectors(const unsigned int,
                                    const VectorType &,
                                    const VectorType &,
...
template <typename VectorType>
inline void
SolverCG1<VectorType>::compute_eigs_and_cond
(const std::vector<double> &diagonal,
 const std::vector<double> &offdiagonal,
 const boost::signals2::signal<void (const std::vector<double> &)> &eigenvalues_signal,
 const boost::signals2::signal<void (double)>                      &cond_signal,
...
template <typename MatrixType, typename PreconditionerType>
void
SolverCG1<VectorType>::solve (const MatrixType         &A,
                             VectorType               &x,
                             const VectorType         &b,
                             const PreconditionerType &precondition)
{
...
//   Vq = this->memory.alloc();
  ...
  std::cout << "       solve function of SolverCG1 accessed" << std::endl;
...
//         std::cout << "    The initial solution is: ";
//         x.print(std::cout);
      ...
      if (!x.all_zero())
        {
          ...
          std::cout << "      x_0 is not a zero vector" << std::endl;
          
          std::cout << "        obtaining A * x0 ";
          A.vmult(g,x);                 // g = A*x0 in SM according to the definition in the system_matrix; or B^(T)*M^(-1)*B*x0 in MM according to the definition in the Schur complement in step-20
                                        // In the CG solver, the residual is the intermediate part to compute the solution. Therefore, the multiplication A*x is necessary here
...
//             g.print(std::cout);   
      ...
      {
          std::cout << "        x_0 is a zero vector" << std::endl;
          g.equ(-1.,b);           // when x0 is all zeros, g (-r0) is -b
      }
...
//       std::cout << "        L2 norm of r0: " << res << std::endl;             // , i.e. b-A*x0,  
...
//       std::cout << "    the type of the precondition recieved in the solve function is: " << typeid(precondition).name() << '\n';
//       std::cout << "    the type of PreconditionIdentity is: " << typeid(PreconditionIdentity).name() << '\n';
//    

      std::cout << "\n";
      std::cout << "        dealing with the precondition" << std::endl;
          
      if (types_are_equal<PreconditionerType,PreconditionIdentity>::value == false)
        {
            
          std::cout << "        the precondition is not PreconditionIdentity" << std::endl;
          
          precondition.vmult(h,g);                  // first, we use the function vmult of InverseMatrix defined in the source file because the preconditioner 'approximate_inverse' is of InverseMatrix type
                                                    // then, since we use the CG solver defined in the above function, the solver would invoke the function vmult in class ApproximateSchurComplement in the source file
          
          std::cout << "        the multiplication of the precondition finished" << std::endl;
      ...
      else
        {
          ...
          std::cout << "        the precondition is PreconditionIdentity" << std::endl;  
          d.equ(-1.,g);         // d is r0
          gh = res*res;         // square of ||r0||_2 
        }
        
        std::cout << "\n";
        
        std::cout << "        r_0: ";               // The residual of the initial solution
        d.print(std::cout);         

//         std::cout << "        square of ||r0||_2, i.e. r0^T*r0: " << gh << std::endl;      
          
//         std::cout << "\n    intermediate iterations ongoing...";
        
//         int last_it = 1;
        Vector <double> custom_residual_vector;  
        Vector <double> custom_intermediate;
        
        double custom_residual_norm_square;
...
//           std::cout << "\n";
          ...
          A.vmult(h,d);                 // h = A * p_k      // when A is the Schur Complement, vmult function involves computing B^(T)*M^(-1)*B*p_k; when A is 'M', it involves M^(-1) * p_k
...
//           std::cout << " <residual>, ";                     // detailed procedure for computing the 
          
          double alpha = d*h;           // p_k^T * A * p_k
          
          std::cout << "          p_" << it-1 << "^T * A * p_" << it-1 << ": " << alpha << std::endl;
          
          
          Assert(alpha != 0., ExcDivideByZero());
          alpha = gh/alpha;             // alpha_k = (r_k^T * r_k)/(p_k^T * A * p_k)
          
          std::cout << "        alpha_" << it-1 << ", i.e. r_" << it-1 << "^T * p_" << it-1 << " / p_" << it-1 << " * A * p_" << it-1 << ": " << alpha << "\n\n";
          
//           std::cout << "     p" << last_it << " is: ";
//           d.print(std::cout,3);
          ...
          x.add(alpha, d);               // x = x + alpha * d, function of Vector in deal.ii
          ...
          custom_residual_vector.reinit(b.size());
          
          
//           ======================= compute ||r << it << ||2
          
//           std::cout << "    ==== 1" << std::endl;                // compute ||r << it << ||2 through b-A*x, which is not accurate
//           
//           A.vmult(custom_residual_vector, x);
//           
//           std::cout << "    A*x" << it << " reads: ";
//           custom_residual_vector.print(std::cout, 3);
//           std::cout << "    b reads: ";
//           b.print(std::cout, 3);
//           
//           custom_residual_vector.add(-1.,b);
//           
//           std::cout << "    -(b-A*x" << it << ") reads: ";
//           custom_residual_vector.print(std::cout, 3);
//           
//           std::cout << "    norm of -(b-A*x" << it << "): " << custom_residual_vector.l2_norm() << std::endl; 
          
//           custom_residual_vector.~Vector();
          
//           std::cout << "    ==== 2" << std::endl;                // detailed process to compute ||r << it << ||2
//           
//           custom_residual_vector = h;
//           custom_residual_vector*=alpha;
//           
//           std::cout << "    alpha" << last_it << "*A*r" << last_it << " is: ";
//           custom_residual_vector.print(std::cout,3);
//           
//           custom_residual_vector += g;              // g is the last -r
//           
//           std::cout << "    alpha" << last_it << "*A*r" << last_it << "-r" << last_it << " is: ";
//           custom_residual_vector.print(std::cout);
//           
//           std::cout << "    ||alpha" << last_it << "*A*r" << last_it << "-r" << last_it << "||2 is: " << custom_residual_vector.l2_norm() << std::endl;
          
// //           const double custom_residual_vector_product = custom_residual_vector*(custom_residual_vector);                                             // another way to compute the L2 norm of a vector, Mar. 26, 2019
// //           std::cout << "    custom_residual_vector_product is: " << custom_residual_vector_product << ", of which the square root is " << std::sqrt(custom_residual_vector_product) << std::endl;
                    
          
//           std::cout << "    ==== 3" << std::endl;               // a simple function to compute ||r << it << ||2
          
          custom_residual_norm_square = g.add_and_dot(alpha, h, g);    // first, r_k = r_k-1 + alpha_k-1 * A * p_k-1; then, r_k^T * r_k
          
          std::cout << "        -r" << it << " is: ";
          g.print(std::cout);           
          
//           std::cout << "    The square of ||r" << it << "||2 is: " << custom_residual_norm_square << std::endl;
          
          res = std::sqrt(custom_residual_norm_square);  
          
          std::cout << "        ||r" << it << "||_2 is: " << res;
          
          std::cout << "\n\n";
          
//           if (/*1*/res < stopping_criterion*1.1)
//           {
//               std::cout << "||r" << it << "||_2 is: " << res;
//           }else
//           {
//               std::cout << "||r" << it << "||_2 is: " << res << ", iteration continued " << std::endl;
//           }
...
//           std::cout << "          preparing for p_" << it << std::endl;               //  << ", alpha"
...
//               std::cout << "    accessed here 2" << std::endl;
          ...
          else
            {
              ...
              beta = gh;                        // r_(k-1)^T * r_(k-1)
              
              std::cout << "          r_" << it-1 << "^T * r_" << it-1 << ": " << beta << std::endl;
          
              gh = res*res;                     // r_(k)^T * r_(k)
              
              std::cout << "          r_" << it << "^T * r_" << it << ": " << gh << std::endl;
              
              beta = gh/beta;                   // beta_(k-1)
              
              std::cout << "          beta_" << it-1 << "^T * beta_" << it-1 << ", i.e. r_" << it << "^T * r_" << it << " / r_" << it-1 << "^T * r_" << it-1 << ": " << beta << std::endl;
              
              d.sadd(beta,-1.,g);               // p_k = p_(k-1) * beta_(k-1) + (-1)*(-r_k)
...
//           std::cout << "    jumps to the next iteration " << std::endl;
        }
    }
  catch (...)
    {
...
template<typename VectorType>
boost::signals2::connection
SolverCG1<VectorType>::connect_coefficients_slot
(const std_cxx11::function<void(double,double)> &slot)
{
...
template<typename VectorType>
boost::signals2::connection
SolverCG1<VectorType>::connect_condition_number_slot
...
template<typename VectorType>
boost::signals2::connection
SolverCG1<VectorType>::connect_eigenvalues_slot
(const std_cxx11::function<void (const std::vector<double> &)> &slot,
 const bool                                                    every_iteration)
{
  if (every_iteration)

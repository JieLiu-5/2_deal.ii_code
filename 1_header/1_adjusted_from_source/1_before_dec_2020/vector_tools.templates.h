...
// ----------- interpolate_boundary_values for std::map --------------------
  ...
  namespace
  {
    ...
    static inline
    void
    do_interpolate_boundary_values                                                                  // Jie, May 10, 2019
    (const M_or_MC<dim, spacedim>                                         &mapping,
     const DoFHandlerType<dim,spacedim>                                   &dof,
     const std::map<types::boundary_id, const Function<spacedim,number>*> &function_map,            // this is to convey the BoundaryValues class, which is stored in the second argument of the map type, which adds one element to function_map
     std::map<types::global_dof_index,number>                             &boundary_values,
     const ComponentMask                                                  &component_mask)
    {
      Assert (component_mask.represents_n_components(dof.get_fe().n_components()),
...
//       std::cout << "    function_map.size(): " << function_map.size() << std::endl;
...
//                 ExcDimensionMismatch(n_components, i->second->n_components));
      ...
      if (dim == 1)
        {
//           std::cout << "    do_interpolate_boundary_values(), 1d case" << std::endl;
          
          
//           std::cout << "    find cells that are on the boundaries of interest" << std::endl;
            ...
            for (unsigned int direction=0;
                 direction<GeometryInfo<dim>::faces_per_cell; ++direction)
                 {
                    ...
                    if (cell->at_boundary(direction)
                        &&
                        (function_map.find(cell->face(direction)->boundary_id()) != function_map.end()))        // this is used to justify if there is a boundary id on the boundary, which can be omitted since there is only one indicator for each boundary, ...
                        {
                            
//                         std::cout << "    @cell " << cell->active_cell_index() << ", face vertex: " << cell->face(direction)->vertex() << ", direction: " << direction << ", the required boundary_id is satisfied" << std::endl;      //cell->face(direction)->
                            
//                         std::cout << "      boundary_id(): " << cell->face(direction)->boundary_id() << std::endl;      //cell->face(direction)->
                            
                        const Function<spacedim,number> &boundary_function = *function_map.find(cell->face(direction)->boundary_id())->second;          // for 2D cases, the boundary condition for different boundary ids might be different, but not in 1d, Jan. 14, 2020
                        
                        // get the FE corresponding to this cell
                        const FiniteElement<dim,spacedim> &fe = cell->get_fe();
                        
//                         std::cout << "    fe.n_components(): " << fe.n_components() << std::endl;
//                         std::cout << "    boundary_function.n_components(): " << boundary_function.n_components << std::endl;
//                         
//                         Assert (fe.n_components() == boundary_function.n_components,
//                                 ExcDimensionMismatch(fe.n_components(),
//                                                     boundary_function.n_components));

//                         std::cout << "    component_mask.n_selected_components(fe.n_components()): " << component_mask.n_selected_components(fe.n_components()) << std::endl;
//                         std::cout << "    component_mask.n_selected_components(): " << component_mask.n_selected_components() << std::endl;
//                         
                        ...
                        Vector<number> function_values (boundary_function.n_components);            // fe.n_components()
                        
                        if (fe.n_components() == 1)
                        {
                            function_values(0) = boundary_function.value (cell->vertex(direction));
//                             std::cout << "    function_values(0): " << function_values(0) << std::endl;
                        }
                        else
                        {
                            boundary_function.vector_value (cell->vertex(direction), function_values);
//                             std::cout << "    function_values: " << function_values << std::endl;
                        }

//                         std::cout << "    fe.dofs_per_vertex: " << fe.dofs_per_vertex << std::endl;
                        
                        for (unsigned int i=0; i<fe.dofs_per_vertex; ++i)
                        {
//                             std::cout << "    fe.face_system_to_component_index(i).first: " << fe.face_system_to_component_index(i).first << ", with the component_mask: " << component_mask[fe.face_system_to_component_index(i).first] << std::endl;
//                             
//                             std::cout << "    fe.face_system_to_component_index(i).second: " << fe.face_system_to_component_index(i).second << std::endl;
//                             
                            if (component_mask[fe.face_system_to_component_index(i).first])                     // not clear, but we always take it ture for 1d case, Jan. 15, 2019
                            {
//                                 std::cout << "    cell->vertex_dof_index(direction, i, cell->active_fe_index()): " << cell->vertex_dof_index(direction, i, cell->active_fe_index()) << std::endl;
                                
                                boundary_values[cell->vertex_dof_index(direction, i, cell->active_fe_index())]
                                    = function_values(fe.face_system_to_component_index(i).first);
                            }
                        }
                                
                        std::cout << "\n    boundary_values: " << std::endl;
...
//           std::cout << "    end of cell looping" << std::endl;
//           std::cout << "    end of do_interpolate_boundary_values(), 1d case" << std::endl;
//           
        }
      else  // dim > 1
        {
...
//     std::cout << "    interpolate_boundary_values 0" << std::endl;
    
//     std::cout << "    types::boundary_id: " << function_map.begin()->first << std::endl;            // it's strange that this does not give boundary_id, Jan/14/2019
//     std::cout << "    types::boundary_id: " << function_map.end()->first << std::endl;
    
//     std::cout << "    component_mask_[0]: " << component_mask_[0] << std::endl;
      
    do_interpolate_boundary_values (mapping, dof, function_map, boundary_values,
                                    component_mask_);
    
//     std::cout << "    end of interpolate_boundary_values 0" << std::endl;
  }
...
//     std::cout << "    interpolate_boundary_values 1" << std::endl;                                   // 
   ...
   const ComponentMask                                                  &component_mask_)
  {
    ...
    std::cout << "    interpolate_boundary_values 2" << std::endl;
  ...
  template <int dim, int spacedim, template <int,int> class DoFHandlerType,
            typename number>
  void
  interpolate_boundary_values                                                   // we probably call this function because the mapping is omitted, Jie
  (const DoFHandlerType<dim,spacedim>       &dof,
   const types::boundary_id                  boundary_component,
   const Function<spacedim,number>          &boundary_function,
   std::map<types::global_dof_index,number> &boundary_values,
...
//     std::cout << "    interpolate_boundary_values 3" << std::endl;
   ...
   const ComponentMask                                                  &component_mask)
  {
    ...
    std::cout << "    interpolate_boundary_values 4" << std::endl;
...
//       std::cout << "      applying compute_face_projection_div_conforming (2d)" << std::endl;
...
//       std::cout << "        vec.first_vector_component: "<< vec.first_vector_component << std::endl;
...
//       std::cout << "        normals at face quadrature points: " << std::endl;
...
//       std::cout << "        face_coordinate_direction: ";
//       for(unsigned int i = 0; i<GeometryInfo<2>::faces_per_cell; ++i)
//       {
//           std::cout << face_coordinate_direction[i] << ", ";
//       }
//       std::cout << std::endl;
...
//       std::cout << "        bountdary values at face quadrature points: " << std::endl;
...
//     std::cout << "        the size of jacobians is: " << jacobians.size() << std::endl;
//     std::cout << "        content: \n";
//     for (unsigned int i=0; i<jacobians.size(); ++i)
//     {
//         for(unsigned int j=0; j<2; ++j)
//         {
//             std::cout << "        " << jacobians[i][j] << ", ";                
//         }
//         std::cout << "\n";
//     }
//     std::cout << std::endl;
...
//           std::cout << std::endl; 
//           std::cout << "        %%% at quadrature " << fe_values.get_quadrature_points ()[q_point] << std::endl;
//           
//           std::cout << "        normals: ";
//           std::cout << normals[q_point];
// 	  
//           std::cout << "        boundary values: ";
// 	  
//             for(unsigned int i = 0; i<values[q_point].size();++i)
//             {
//                 std::cout << values[q_point][i] << ", ";
//             }

//           values[q_point].print(std::cout);	  

            
          double tmp = 0.0;

          for (unsigned int d = 0; d < 2; ++d)              // compute u * n at each quadrature point of the face
          {
            tmp += normals[q_point][d] * values[q_point] (d);
          }
          
//           std::cout << "      tmp (u*n): " << tmp;                     // normal component of the boundary condition
//           std::cout << std::endl;          
//           
//           std::cout << "        jacobians: ";
//           for(unsigned int j=0; j<2; ++j)
//           {
//               std::cout << jacobians[q_point][j] << ", ";                
//           }
//           
//           
//           std::cout << "with determinant: " << jacobians[q_point].determinant();
//           
//           
//           std::cout << "        fe_values.JxW (q_point): " << fe_values.JxW (q_point);
// 	  
//           std::cout << "        sqrt_J: " << 
...
//           
          tmp *= fe_values.JxW (q_point)
                 * std::sqrt (jacobians[q_point][0][face_coordinate_direction[face]]                                // to pick x or y part of the first row of the jacobians
                              * jacobians[q_point][0][face_coordinate_direction[face]]
                              + jacobians[q_point][1][face_coordinate_direction[face]]
                              * jacobians[q_point][1][face_coordinate_direction[face]]);
// 
//           std::cout << "      tmp (u*n*JW*sqrt_J): " << tmp;                     // normal component of the boundary condition
//           std::cout << std::endl;  
	  
//           std::cout << "        fe_values[vec].value (fe.face_to_cell_index (i, face), q_point): " << std::endl;
...
// 	  std::cout << std::endl;
          ...
          {
//             std::cout << "        face dof ";
//             std::cout << fe.face_to_cell_index (i, face);
//             std::cout << ", with values: " << fe_values[vec].value (fe.face_to_cell_index (i, face), q_point);
//             
//             std::cout << ", contribution: " << tmp * (normals[q_point] * fe_values[vec].value (fe.face_to_cell_index (i, face), q_point));
	    
            dof_values (i) += tmp * (normals[q_point]							// contributionn of per quadrature point to each dof_values
                                     * fe_values[vec].value (fe.face_to_cell_index (i, face),
                                                             q_point));
//             std::cout << "        resulting dof_values (" << i << "): " << dof_values (i) << std::endl;
          }
//           std::cout << std::endl;
...
//         std::cout << std::endl;
        ...
        if (!(constraints.is_constrained (face_dof_indices[i])))
          {
            ...
            {
              constraints.set_inhomogeneity (face_dof_indices[i], dof_values (i));             // dof_values (i)
            }
          }
    }
                                          ...
                                          const Mapping<dim>       &mapping)
  {
    ...
    hp::FEValues<dim> fe_values (mapping_collection, fe_collection, quadrature_collection, update_quadrature_points | update_jacobians);                             // pay attention to | in the flags
...
//         std::cout << "    I'm in vector_tools.templates.h -> function: project_boundary_values_div_conforming" << std::endl;                 // Jie
...
//                 std::cout << "    Cell no.: " << cell->active_cell_index() << std::endl;   
                ...
                if (cell->at_boundary ())
                    ...
                    if (cell->face (face)->boundary_id () == boundary_component)
                        {
//                             std::cout << "      face no.(ordinal): " << face;
//                             std::cout << "      coords: ";
//                             for (unsigned int vertex=0; vertex < GeometryInfo<dim>::vertices_per_face; ++vertex)
//                             {
//                                 std::cout << " (" << cell->face(face)->vertex(vertex) << ")";                   // 
//                             }                            
//                             std::cout << "      face_index: " << cell->face_index(face);
//                             std::cout << std::endl;
...
//                                 std::cout << "      dynamic_cast<const FESystem<dim>*> (&cell->get_fe ()) == 0" << std::endl;
...
//                         std::cout << "      cell->get_fe ().get_name(): " << cell->get_fe().get_name() << std::endl;
                        
//                         std::cout << "    cell->active_fe_index (): " << cell->active_fe_index () << std::endl;
                        
                        fe_values.reinit (cell, face + cell->active_fe_index ()                         // here we designate which face of the cell to be reinitialized
                                            * GeometryInfo<dim>::faces_per_cell);
                        
//                         std::cout << "      fe_values.get_present_fe_values ().get_quadrature_points()" << std::endl;                          //
//                         for(unsigned int i =0; i<fe_values.get_present_fe_values ().get_quadrature_points().size(); ++i)
//                         {
//                             std::cout << "      " << fe_values.get_present_fe_values ().get_quadrature_points()[i] << std::endl;
//                         }

                        const std::vector<DerivativeForm<1,dim,spacedim> > &
                        jacobians = fe_values.get_present_fe_values ().get_jacobians ();                // jacobians of the quadrature points on the face
                        
                        
        //                   std::cout << "    jacobians: " << std::endl;
        //                   for (unsigned int i=0; i<jacobians.size(); ++i)
        //                   {
        //                       std::cout << jacobians[i].determinant() << std::endl;
        //                   }
...
//                         std::cout << "      fe_face_values.get_quadrature_points()" << std::endl;                          //
//                         for(unsigned int i =0; i<fe_face_values.get_quadrature_points().size(); ++i)
//                         {
//                             std::cout << "      " << fe_face_values.get_quadrature_points()[i] << std::endl;
//                         }
  ...
  namespace internal
  {
    ...
    template <int dim, int spacedim, typename Number>                                                                                               // IDScratchData
    ...
    {
//         std::cout << "    constructor of IDScratchData\n";
        
//         std::cout << "    q.size(): " << q.size() << "\n";
//         std::cout << "    q[0].size(): " << q[0].size() << "\n";
//         
//         std::cout << "    q[0].get_points(): ";
//         for (auto i = q[0].get_points().begin(); i<q[0].get_points().end(); ++i)
//         {
//             std::cout << *i << " ";
//         }
//         std::cout << "\n";
    ...
    integrate_difference_inner (const Function<spacedim>   &exact_solution,
                                ...
                                const UpdateFlags            update_flags,      // this is determined based on the norm type received in function 'integrate_difference', Sep 10, 2019
                                const double                 exponent,
                                const unsigned int           n_components,
                                IDScratchData<dim,spacedim,Number> &data)
    {

//       std::cout << "      integrate_difference_inner()" << std::endl;
      
      const bool fe_is_system = (n_components != 1);
      const dealii::FEValues<dim, spacedim> &fe_values  = data.x_fe_values.get_present_fe_values ();
      const unsigned int n_q_points = fe_values.n_quadrature_points;                        // this data has been used in function 'do_integrate_difference'
...
//       std::cout << "\n";
//       
//       std::cout << "      weight_vectors, of size " << data.weight_vectors.size();
//       std::cout << ", reading \n";
//       for (unsigned int k=0; k<n_q_points; ++k)
//       {
//           std::cout << "      [" << k << "] " << data.weight_vectors[k];
//       }
//       std::cout << std::endl;
//     
// 
//       std::cout << "      evaluating exact solution at quadrature points(psi_xx) based on flags and do the substraction operation\n";

      if (update_flags & update_values)                                                           // before computing the norms, we first calculate the difference of the quantity at quadrature points
        {
...
//           std::cout << "After initialization, psi_values reads " << std::endl;
                      
          if (fe_is_system)
            {
                
//               std::cout << "fe_is_system" << std::endl;                
...
/*          for (unsigned int i=0; i<n_q_points; ++i)
          {
              for (unsigned int comp=0; comp<data.psi_values[i].size(); ++comp)
              {              
                  std::cout << "data.psi_values[" << i << "][" << comp << "]: " << data.psi_values[i][comp] << std::endl;
              }
          }*/              
...
/*          std::cout << "after subtracting function_values, psi_values becomes" << std::endl;
...
//           std::cout << "After initialization, the gradients of the exact solution at quadrature points read " << std::endl;
...
//             std::cout << "      updating normal vectors\n";
...
//             std::cout << "      not updating normal vectors\n";
...
//               std::cout << "      component " << k << "\n";
              ...
              for (unsigned int q=0; q<n_q_points; ++q)
                ...
                {
//                   std::cout << "      data.psi_grads[" << q << "][" << k << "][" << d << "]: " << data.psi_grads[q][k][d];
//                   std::cout << " substracted by function_grads " << data.function_grads[q][k][d];
//                   
                  data.psi_grads[q][k][d] -= data.function_grads[q][k][d];
                  
//                   std::cout << " equals " << data.psi_grads[q][k][d] << std::endl;
                }
            }
          }
                
/*          std::cout << "after subtracting function_grads, psi_grads becomes" << std::endl;
          
            for (unsigned int i=0; i<n_q_points; ++i)
            {
                for (unsigned int comp=0; comp<data.psi_grads[i].size(); ++comp)
                {
                    std::cout << "data.psi_grads[" << i << "][" << comp << "]: " << data.psi_grads[i][comp] << std::endl;
                }
            }   */              
      ...
      if (update_flags & update_hessians)
        {  
            ...
            for (unsigned int i=0; i<n_q_points; ++i)
            {
                    //std::cout << data.tmp_gradients[i] << " " << data.tmp_hessians[i] << std::endl;
            ...
            for (unsigned int k=0; k<n_components; ++k)
                for (unsigned int q=0; q<n_q_points; ++q)
                    for (unsigned int d1=0; d1<spacedim; ++d1)
                        ...
                        {
                                data.psi_hessians[q][k][d1][d2] -= data.function_hessians[q][k][d1][d2];
                        }
        }
...
//           std::cout << "    content of the mean norm: ";
          ...
          for (unsigned int q=0; q<n_q_points; ++q)
            {
              ...
              for (unsigned int k=0; k<n_components; ++k)
              {
//                 std::cout << double(data.psi_values[q](k)* data.weight_vectors[q](k)) << " ";
...
//                 std::cout << "data.psi_values[" << q << "][" << k << "]: " << data.psi_values[q][k];
                
                sum += numbers::NumberTraits<Number>::abs_square(data.psi_values[q](k)) *
                       data.weight_vectors[q](k);
              }
              
//               std::cout << "     fe_values.JxW(" << q << "): " << fe_values.JxW(q) << std::endl; 
              
//               std::cout << "     fe_values.jacobian(" << q << "): " << fe_values.jacobian(q).determinant() << std::endl;                 // not done, Sep. 11, 2019
...
//             std::cout << std::endl;
          ...
          if (norm == L2_norm)
            diff=std::sqrt(diff);                                                                           // we compute the square root because we use l2_norm() for cellwise_error later in line 7866
          break;

        case Linfty_norm:
        case W1infty_norm:
        {
          std::cout << "    content of Linfty norm: ";
          
          for (unsigned int q=0; q<n_q_points; ++q)
            for (unsigned int k=0; k<n_components; ++k)
            {
              std::cout << double(std::abs(data.psi_values[q](k)* data.weight_vectors[q](k))) << " ";
              diff = std::max (diff, double(std::abs(data.psi_values[q](k)*                                 // choose the largest absolute value of the solution at the quadrature points multiplied by the weight
                                                     data.weight_vectors[q](k))));
            }
          break;
        }
...
//                 std::cout << "data.psi_grads[" << q << "][" << k << "]: " << data.psi_grads[q][k] << std::endl;             //  << ", [0]: " << data.psi_grads[q][k][0] << ", [1]: " << data.psi_grads[q][k][1] 
                
                sum += data.psi_grads[q][k].norm_square() *
                       data.weight_vectors[q](k);                             // weight_vectors tells us which component is of interest
                       
//                 std::cout << "      sum: " << sum << "\n";
                       
/*                sum += pow(abs(data.psi_grads[q][k][0]),2) *                // only computing the error of ux, Sep. 11, 2019
                       data.weight_vectors[q](k);  */         
                       
/*                sum += pow(abs(data.psi_grads[q][k][1]),2) *                // only computing the error of uy
                       data.weight_vectors[q](k);*/                       
      ...
      switch (norm)
      {
        case H2_seminorm:
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                ...
                for (unsigned int k=0; k<n_components; ++k)
                {
                    sum += data.psi_hessians[q][k].norm_square() *
                        data.weight_vectors[q](k);
                }
                diff += sum * fe_values.JxW(q);                
            }
    ...
    do_integrate_difference (const dealii::hp::MappingCollection<dim,spacedim> &mapping,
                             ...
                             const InVector                                    &fe_function,                // the numerical solution is used to get the values at quadrature points, which are stored in 'data.function_values' in this function
...
//       std::cout << "    internal::do_integrate_difference()" << std::endl;
...
//           std::cout << "    weight!=0\n";
//           std::cout << "    weight->n_components: " << weight->n_components << std::endl;
...
//       std::cout << "\n\n";
      ...
      // loop over all cells
      for (typename DoFHandlerType::active_cell_iterator cell = dof.begin_active();                                                                     // looping over each cell for do_integrate_difference()
           cell != dof.end(); ++cell)
        if (cell->is_locally_owned())
          {
...
//             std::cout << "    the basic values to compute the norm are " << std::endl;
            
              
            data.x_fe_values.reinit (cell);
            
//             std::cout << "    data.x_fe_values.JxW(0): " << data.x_fe_values.JxW(0) << std::endl;
//             std::cout << "    data.x_fe_values.get_quadrature_collection()[0].point(0): " << data.x_fe_values.get_quadrature_collection()[0].point(0) << std::endl;                 

            const dealii::FEValues<dim, spacedim> &fe_values
              = data.x_fe_values.get_present_fe_values ();
              
            
//             std::cout << "    fe_values.get_quadrature_points(): ";
//             for (auto i = fe_values.get_quadrature_points().begin(); i<fe_values.get_quadrature_points().end(); ++i)
//             {
//                 std::cout << *i << " ";
//             }
//             std::cout << "\n";
//             
//             std::cout << "    fe_values.get_JxW_values(): ";
//             for (auto i = fe_values.get_JxW_values().begin(); i<fe_values.get_JxW_values().end(); ++i)
//             {
//                 std::cout << *i << " ";
//             }
//             std::cout << "\n";
...
//             std::cout << "      setting the size for function_xx, weight_xx, psi_xx, tmp_xx\n";

            data.resize_vectors (n_q_points, n_components);                                             // the size of the vector of basis functions at quadrature points is assigned here, Sep. 11, 2019
                                                                                                        // set the size for function_xx, weight_xx, psi_xx, tmp_xx

//           std::cout << "      data.psi_values.size(): " << data.psi_values.size() << ", ";              // the size of 'psi_values' has been assigned in function 'do_integrate_difference'
//           std::cout << "psi_values[0].size(): " << data.psi_values[0].size() << std::endl;
//           std::cout << "      data.psi_grads.size(): " << data.psi_grads.size() << ", ";              
//           std::cout << "psi_grads[0].size(): " << data.psi_grads[0].size() << std::endl;
//           std::cout << "      data.psi_hessians.size(): " << data.psi_hessians.size() << ", ";              
//           std::cout << "psi_hessians[0].size(): " << data.psi_hessians[0].size() << std::endl;
//                         
//           std::cout << "\n";
          
//             std::cout << "data.function_values.size(): " << data.function_values.size() << ", ";             
//             std::cout << "data.function_values[0].size(): " << data.function_values[0].size() << std::endl;
//                 
//             std::cout << "data.function_grads.size(): " << data.function_grads.size() << ", ";             
//             std::cout << "data.function_grads[0].size(): " << data.function_grads[0].size() << std::endl;
//                     
//             std::cout << "data.function_hessians.size(): " << data.function_hessians.size() << ", ";             
//             std::cout << "data.function_hessians[0].size(): " << data.function_hessians[0].size() << std::endl;
                
                
//             std::cout << "      evaluating numerical solution at quadrature points(function_xx) based on flags\n";
...
//                 std::cout << "    extracting the values of the numerical solution at quadrature points\n";
//                 std::cout << "    the values of the numerical solution at quadrature points reads" << std::endl;
//                 for (unsigned int q=0; q<n_q_points; ++q)
//                 {
//                     for (unsigned int i=0; i<data.function_values[q].size(); ++i)
//                     {
//                         std::cout << "    data.function_values[" << q << "][" << i << "]: " << data.function_values[q][i] << std::endl;  
//                     }
//                 }
...
/*                std::cout << "    extracting the first derivatives of the numerical solution at quadrature points\n";
                std::cout << "    the gradient of the numerical solution at quadrature points reads" << std::endl;
                
                for (unsigned int q=0; q<n_q_points; ++q)
                {
                    for (unsigned int i=0; i<data.function_grads[q].size(); ++i)
                    {
                        std::cout << "    data.function_grads[" << q << "][" << i << "]: " << data.function_grads[q][i] << std::endl;  
                    }
                }     */         
...
/*                std::cout << "    extracting the second derivatives of the numerical solution at quadrature points\n";
                for (unsigned int q=0; q<n_q_points; ++q)
                {
                    for (unsigned int i=0; i<data.function_hessians[q].size(); ++i)
                    {
                        std::cout << "    data.function_hessians[" << q << "][" << i << "]: " << data.function_hessians[q][i] << std::endl;  
                    }
                }    */     
...
//             std::cout << std::endl;
            ...
            difference(cell->active_cell_index()) =                                                 // here we store the difference in each cell to 'difference'
...
//         std::cout << "\n";
  ...
  integrate_difference (const DoFHandler<dim,spacedim> &dof,    
                        const InVector                 &fe_function,
                        const Function<spacedim>       &exact_solution,             // ,Number
                        OutVector                      &difference,
                        const Quadrature<dim>          &q,
                        const NormType                 &norm,
                        const Function<spacedim>       *weight,             // ,Number
                        const double                    exponent)           // exponent is only used in Lp, L1, and W1p kind of norms in integrate_difference_inner().
  {
      
//     std::cout << "    vector_tools::integrate_difference()" << std::endl;
...
//         return local;                                // we can return local directly, which has the same value with returning the following line

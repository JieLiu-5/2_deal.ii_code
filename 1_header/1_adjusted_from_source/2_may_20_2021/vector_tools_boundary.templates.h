...
//       std::cout << "internal::do_interpolate_boundary_values()\n";
...
//         std::cout << "i->second->n_components: " << i->second->n_components << "\n";
  ...
  {
//     std::cout << "VectorTools::interpolate_boundary_values(), function_map assigned\n";
...
//     std::cout << "VectorTools::interpolate_boundary_values(), mapping assigned\n";
...
//     std::cout << "VectorTools::interpolate_boundary_values()\n";
  ...
  namespace internals
  {
    ...
    template <typename cell_iterator>
    void
    compute_face_projection_div_conforming_rt(
      const cell_iterator &                       cell,
      const unsigned int                          face,
    ...
    template <int dim, typename cell_iterator>
    void
    compute_face_projection_div_conforming_bdm                              // important for dim=1, jan 6, 2021
    (const cell_iterator &,
     const unsigned int ,
     const FEFaceValues<dim> &,
     const unsigned int ,
     const Function<dim> &,
     const DerivativeForm<1,dim, dim> &,
     AffineConstraints<double> &)
    {
      Assert(false, ExcNotImplemented());
//         if(dim == 1)
//         {
//             std::cout << "dimension cannot be 1 for bdm elements\n";
//             throw std::exception();
//         }
    }    
    ...
    (const cell_iterator                       &cell,
     ...
     const unsigned int                         first_vector_component,
     const Function<2>                         &boundary_function,
     const DerivativeForm<1,2, 2>               &jacobian_of_one_quadrature_point,
     AffineConstraints<double>                          &constraints)
    {
      ...
      const unsigned int face_coordinate_direction[GeometryInfo<2>::faces_per_cell] = {0, 0, 1, 1};
      ...
      param_jacobian = std::sqrt (jacobian_of_one_quadrature_point[0][face_coordinate_direction[face]]
      ...
      for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
      {
        dof_values (i) = values[i][face_coordinate_direction[face]]*param_jacobian;
      }
  ...
  template <int dim>
  void
  project_boundary_values_div_conforming_rt(
    const DoFHandler<dim> &    dof_handler,
    const unsigned int         first_vector_component,
    const Function<dim> &      boundary_function,
    const types::boundary_id   boundary_component,
                  ...
                  if (cell->face(face)->boundary_id() == boundary_component)
                    {
                      ...
                      internals::compute_face_projection_div_conforming_rt(
                        cell,
                        face,
                        fe_face_values,
                        first_vector_component,
  ...
  template <int dim>
  void
  project_boundary_values_div_conforming_bdm (const DoFHandler<dim>    &dof_handler,
                                          const unsigned int        first_vector_component,
                                          const Function<dim>      &boundary_function,
                                          const types::boundary_id  boundary_component,
                                          AffineConstraints<double>         &constraints,
      ...
      if (cell->at_boundary ())
      {
        ...
        for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
        {
          if (cell->face (face)->boundary_id () == boundary_component)
          {
            ...
            fe_values.reinit (cell, face + cell->active_fe_index () * GeometryInfo<dim>::faces_per_cell);

            const DerivativeForm<1,dim,spacedim>& jacobian_of_one_quadrature_point = fe_values.get_present_fe_values ().get_jacobians ()[0];  

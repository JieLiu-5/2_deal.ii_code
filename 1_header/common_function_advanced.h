...
template <int dim>
void print_coords_of_dofs(string obj_string,
...
template <int dim>
void putting_coords_of_dofs_into_a_vector(DoFHandler<dim>& dof_handler,
                                          vector<Point<dim> >& vector_coords,
                                          unsigned int current_refinement_level)
{
  cout << "  Saving coordinates of dofs of " << current_refinement_level << "th refinement to a file\n";
  
  MappingQ<dim, dim> mapping(dof_handler.get_fe().degree);            
  DoFTools::map_dofs_to_support_points(mapping, dof_handler, vector_coords);
}


template <typename T>
void save_system_matrix_to_txt(string& obj_string,
                               SparseMatrix<T> &obj_matrix)
{
  cout << "Saving " << obj_string << " to a file\n";
  ofstream fid(obj_string+".txt");
...
template <int dim>
void print_constraints_info(string& obj_string,
...
void extract_basis_function (T& fe)                                // only available for one-dimensional case, Aug. 28, 2020
{
  ...
  obj_string_file_name = "data_unit_coords.txt";
  saving_vector_of_Point_to_a_file(obj_string_file_name,
                              coords_interpolate);

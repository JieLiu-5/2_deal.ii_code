...
//     cout << "coeff_values: \n";
//     print_vector_vertically(coeff_values);
    
    for (unsigned int q=0; q</*1*/n_q_points; ++q)
    {
      cellwise_inte_coeff_square[0]+= pow(abs(coeff_values[q].real()), 2.0) * fe_values.JxW(q);  
      cellwise_inte_coeff_square[1]+= pow(abs(coeff_values[q].imag()), 2.0) * fe_values.JxW(q);  
    }
    cellwise_sqrt_inte_coeff_square[cell->active_cell_index()][0] += sqrt(cellwise_inte_coeff_square[0]);
    cellwise_sqrt_inte_coeff_square[cell->active_cell_index()][1] += sqrt(cellwise_inte_coeff_square[1]);

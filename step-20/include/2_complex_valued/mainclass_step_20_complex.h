...
#include <deal.II/lac/affine_constraints_custom_function.h>


#include"../../6_headers/1_cpp/1_dealii/base_class_complex.h"
#include<schur_complement_step_20.h>
#include<../../step-4/include/2_complex_valued/exact_solution_step_4_complex.h>
#include<2_complex_valued/exact_solution_and_gradient_boundary_step_20_complex.h>
#include<2_complex_valued/zero_function_step_20_complex.h>
...
class MixedLaplaceProblem_Complex
{
  ...
  private:
    ...
    unsigned int is_constraints_printed = 0;
    ...
    unsigned int id_fe_system = 0;                   // '0' for Q/DGQ, '1' for either RT/DGQ or BDM/DGP,       
...
//     print_constraints_info(obj_string, constraints_auxiliary, dof_handler);
  ...
  const std::vector<types::global_dof_index> dofs_per_component_real = DoFTools::count_dofs_per_fe_component(dof_handler);  
...
//     cout << "  dofs_per_component_real: \n";
//     print_vector(dofs_per_component_real);
...
//   vector<Tensor<2,dim, complex<double>>> coeff_diff_values(n_q_points,Tensor<2,dim,complex<double>>());
  vector<Tensor<2,dim, complex<double>>> coeff_diff_inverse_values(n_q_points,Tensor<2,dim,complex<double>>());
...
//   print_coords_of_dofs(obj_string,dof_handler);
  ...
  if(is_l2_norm_coefficient_numerical == 1 && n_times_called_assemble==0)
  {
    ...
    compute_l2_norms_of_coefficient_numerically_core_tensor_complex(triangulation, dof_handler, fe_values, obj_coeff_diff, n_number_components);
...
//     cout << "  Cell #: " << cell->active_cell_index() << endl;
    ...
    right_hand_side.vector_values_rhs(coords_of_quadrature, rhs_values);
//     obj_coeff_diff.value(coords_of_quadrature, coeff_diff_values);
    obj_coeff_diff.inverse(coords_of_quadrature, coeff_diff_inverse_values);
...
//         cout << "  phi_i_pressure_imag: " << phi_i_pressure_imag << "\n";
        ...
        for (unsigned int j=0; j<dofs_per_cell; ++j)
        {
          ...
          if (fe.system_to_base_index(i).first.first == 0)
          {
            ...
            {
              local_matrix(i,j) += coeff_diff_inverse_values[q][0][0].real() *(phi_i_velocity_real * phi_j_velocity_real) * fe_values.JxW(q);
            ...
            {
              local_matrix(i,j) += - coeff_diff_inverse_values[q][0][0].imag() * (phi_i_velocity_real * phi_j_velocity_imag) * fe_values.JxW(q);
            }
          }else if(fe.system_to_base_index(i).first.first == 1)
          {
            ...
            {
              local_matrix(i,j) += - phi_i_pressure_real * (coeff_helm_values[q].real() * phi_j_pressure_real) * fe_values.JxW(q);
            }else if(fe.system_to_base_index(j).first.first == 3)
            {
              local_matrix(i,j) += phi_i_pressure_real * (coeff_helm_values[q].imag() * phi_j_pressure_imag) * fe_values.JxW(q);
            }
          ...
          }else if(fe.system_to_base_index(i).first.first == 2)
          {
            if(fe.system_to_base_index(j).first.first == 0)
            {
              local_matrix(i,j) += coeff_diff_inverse_values[q][0][0].imag() * (phi_i_velocity_imag * phi_j_velocity_real) * fe_values.JxW(q);
            }else if(fe.system_to_base_index(j).first.first == 2)
            {
              local_matrix(i,j) += coeff_diff_inverse_values[q][0][0].real() * (phi_i_velocity_imag * phi_j_velocity_imag) * fe_values.JxW(q);
            }else if(fe.system_to_base_index(j).first.first == 3)
            {
          ...
          }else if(fe.system_to_base_index(i).first.first == 3)
          {
            if(fe.system_to_base_index(j).first.first == 1)
            {
              local_matrix(i,j) += - phi_i_pressure_imag * (coeff_helm_values[q].imag() * phi_j_pressure_real) * fe_values.JxW(q);
            }else if(fe.system_to_base_index(j).first.first == 2)
            {
              local_matrix(i,j) += - phi_i_pressure_imag * div_phi_j_velocity_imag * fe_values.JxW(q);
            }else if(fe.system_to_base_index(j).first.first == 3)
            {
              local_matrix(i,j) += - phi_i_pressure_imag * (coeff_helm_values[q].real() *phi_j_pressure_imag) * fe_values.JxW(q);
            }
          }
...
void MixedLaplaceProblem_Complex<dim>::compute_errors_and_l2_norms_built_in ()
{
  ...
  VectorTools::integrate_difference (dof_handler_single, solution_0, exact_solution_real,
...
void MixedLaplaceProblem_Complex<dim>::print_errors_to_file()
{
  ...
  myfile.open ("data_error_"+obj_string+"_mm_complex.txt", ofstream::trunc);

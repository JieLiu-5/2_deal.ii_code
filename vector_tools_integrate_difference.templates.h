            ...
            for (unsigned int k = 0; k < n_components; ++k)
              for (unsigned int q = 0; q < n_q_points; ++q)
                for (unsigned int d = 0; d < spacedim; ++d)
                  data.psi_grads[q][k][d] -= data.function_grads[q][k][d];                              // function_grads, which is the values of the numerical solution at quadrature points, is initialized in do_integrate_difference() through
        ...
        if (cell->is_locally_owned())
          {
            ...
            if (update_flags & update_gradients)                                                            // initializing gradients of the numerical solution of the variable (data.function_grads)
